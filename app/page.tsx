    ...
    <div className="flex h-screen">
      <div className="w-1/3 bg-white border-r p-4 overflow-y-auto">
        ...
        {uniqueSenders.map(sender => (
          <div
            key={sender}
            className={`p-3 cursor-pointer rounded-lg mb-2 ${selectedSender === sender ? 'bg-blue-100' : 'hover:bg-gray-100'} ${unreadSenders.has(sender) ? 'font-bold' : ''}`}
            onClick={() => {
      ...
      <div className="w-2/3 flex flex-col p-4">
        {selectedSender ? (
          ...
          <p className="text-gray-500 text-center mt-20">Selecione um cliente para visualizar a conversa</p>
        )}
      </div>
    </div>
  )
